  ...
  <div class="header">
    <v-row class="align-center px-5 py-1" style="z-index: 100 !important">
      <v-row cols="1" class="pa-0 ma-0 align-center d-flex">
        ...
        <label for="" class="ml-4 black100t">{{ title }}</label>

        <v-menu offset-y>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              flat
              class="transparent blue100t"
              elevation="0"
              small
              v-bind="attrs"
              v-on="on"
          ...
          <v-list class="black500">
            <v-list-item v-for="(item, index) in idiomas" :key="index">
              <v-list-item-title class="black100t" @click="mudarIdioma(item)">{{
                item.name
              }}</v-list-item-title>
            </v-list-item>
          </v-list>
      ...
      <v-col cols="7" class="dragable ma-0"> </v-col>
...
export default {
  ...
  methods: {
    ...
    mudarIdioma(idioma) {
      if (idioma.key == localStorage.lang) return;
