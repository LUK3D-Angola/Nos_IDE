<template>
  <v-col
    class="node pa-0 ma-0"
    :class="[selected(), node.name] | kebab; node.name"
  >
    <v-row class="pa-0 ma-0 justify-center align-center">
      ...
      <div class="inputs">
        <div class="input" v-for="input in inputs()" :key="input.key">
          <Socket
            ...
            type="input"
            class="bblack600 whiteb"
            :socket="input.socket"
          ></Socket>
          <div class="input-title black100t" v-show="!input.showControl()">
            {{ input.name }}
          </div>
          <div
            class="input-control"
      ...
      <v-tooltip
        bottom
        :stype="{ 'opacity:0;': destroyed != false }"
        @mouseleave.prevent="destroyed = !destroyed"
      >
        <template v-slot:activator="{ on, attrs }">
          <div class="title black100t" v-bind="attrs" v-on="on">
