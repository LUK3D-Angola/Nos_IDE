  ...
  <div class="home">
    ...
    <v-col>
      <v-text-field
        v-model="projectName"
        :rules="rules.name"
        @mouseover="tooltip = $t('what_is_your_project_name')"
        @mouseleave="tooltip = ''"
        background-color="#0C0C0D"
        class="black300t projectName"
        color="#ffffff"
        flat
        dense
        solo
      ...
      <v-row class="pl-3">
        <v-icon class="black200t pr-3">mdi-folder-outline</v-icon>
        <v-chip
          :class="`${selected ? 'primary' : 'primary'}`"
          @click="selectFolder()"
        >
          <label
            for=""
            id="caminho"
            @mouseover="tooltip = $t('select_folder_for_your_project')"
            @mouseleave="tooltip = ''"
      ...
      <v-row>
        ...
        <v-col cols="6">
          <v-text-field
            ...
            @mouseover="tooltip = 'The versio of the project'"
            @mouseleave="tooltip = ''"
            background-color="#0C0C0D"
            class="black300t"
            color="#ffffff"
      ...
      <v-row>
        <v-col cols="6">
          <v-btn @click="createProject()" small color="primary elevation-0"
            >Start Project</v-btn
          >
        </v-col>
      </v-row>
    ...
    <v-footer
      absolute
      class="font-weight-medium black500 black200t pa-0 px-3"
      color="#0C0C0D"
    >
      <v-icon class="yellow100t">mdi-alert-box-outline</v-icon>
      <v-subheader class="pa-0 ma-0 black200t">{{ tooltip }}</v-subheader>
    </v-footer>
  </div>
</template>
    ...
    return {
      ...
      rules: {
        name: [
          ...
          (val) =>
            (val || "").split(" ").length <= 1 ||
            "Remove the blank Space or replace it with _",
        ],
      },
    };
  },
